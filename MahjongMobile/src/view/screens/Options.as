package view.screens 
{
	...
	public class Options extends Screen 
	{
		...
		// Mobile only
		private var _toggleZoom:ToggleSwitch;
		private var _textZoom:ITextRenderer;
		private var _toggleTouch:ToggleSwitch;
		private var _textTouch:ITextRenderer;
		...
		override protected function initialize():void
		{
			...
			// Mobile only
			this._toggleTouch = new ToggleSwitch();
			this._toggleTouch.isSelected = false;
			this._toggleTouch.onChange.add(toggleTouch_onChange);
			this.addChild(_toggleTouch);
			
			this._textTouch = FeathersControl.defaultTextRendererFactory();
			this._textTouch.text = "Offset touch for mobiles";
			this.addChild(DisplayObject(this._textTouch));
			
			this._toggleZoom = new ToggleSwitch();
			this._toggleZoom.isSelected = false;
			this._toggleZoom.onChange.add(toggleZoom_onChange);
			this.addChild(_toggleZoom);
			
			this._textZoom = FeathersControl.defaultTextRendererFactory();
			this._textZoom.text = "Enable zooming";
			this.addChild(DisplayObject(this._textZoom));
		...
		override protected function draw():void
		{
			...
			//Mobile
			const contentHeight:Number = 4 * (lineHeight + spacingY);
			...
			// Mobile only
			this._toggleTouch.validate();
			const displayTouchLabel:FeathersControl = FeathersControl(this._textTouch);
			displayTouchLabel.validate();
			this._toggleZoom.validate();
			const displayZoomLabel:FeathersControl = FeathersControl(this._textZoom);
			
			_toggleTouch.x = (this.actualWidth - contentWidth) / 2;
			_toggleTouch.y = this._toggleSound.y + this._toggleSound.height + spacingY;
			displayTouchLabel.x = this._toggleTouch.x + this._toggleTouch.width + spacingX;
			displayTouchLabel.y = this._toggleTouch.y + (this._toggleTouch.height - displayValueSound.height) / 2;
			
			_toggleZoom.x = (this.actualWidth - contentWidth) / 2;
			_toggleZoom.y = this._toggleTouch.y + this._toggleTouch.height + spacingY;
			displayZoomLabel.x = this._toggleZoom.x + this._toggleZoom.width + spacingX;
			displayZoomLabel.y = this._toggleZoom.y + (this._toggleZoom.height - displayValueSound.height) / 2;
		}
		...
		private function toggleTouch_onChange(toggleSwitch:ToggleSwitch):void
		{
			Constants.TOUCH_ENABLED = toggleSwitch.isSelected;
		}
		
		private function toggleZoom_onChange(toggleSwitch:ToggleSwitch):void
		{
			Constants.ZOOM_ENABLED = toggleSwitch.isSelected;
		}
